...
const CROP_PROFILES = [
  { id:1, emoji:"🍅", name:"Tomatoes (Summer)", type:"Solanum lycopersicum", assignedTo:"BATCH-001",
    params:[{name:"Soil Moisture",min:60,max:80,unit:"%",pct:70},{name:"Temperature",min:21,max:29,unit:"°C",pct:60},{name:"Humidity",min:60,max:75,unit:"%",pct:68},{name:"Soil pH",min:6.0,max:6.8,unit:"",pct:55}],
...
function CropProfilesPage() {
  ...
  return (
    <>
      ...
      <div className="fs-stat-strip">
        <div className="fs-stat-card fs-stat-card--gold"><div className="fs-stat-card__label">Active Profiles</div><div className="fs-stat-card__val">{CROP_PROFILES.length}</div><div className="fs-stat-card__meta">Custom recipes</div><span className="fs-stat-tag fs-stat-tag--good">All active</span></div>
      ...
      <div className="fs-grid-2" style={{alignItems:"start"}}>
        <div>
          <div className="fs-section-row"><div className="fs-section-label">My Crop Recipes</div></div>
            ...
            {CROP_PROFILES.map((p, i) => (
              <div key={p.id} className={`fs-profile-card${selected===p.id?" ":" "}`} style={{animationDelay:`${0.05+i*0.07}s`,outline:selected===p.id?`2px solid var(--gold)`:"none",outlineOffset:2}} onClick={() => setSelected(selected===p.id?null:p.id)}>
                ...
                <div className="fs-profile-card__footer">
                  ...
                  <button className="fs-btn fs-btn--ghost fs-btn--sm" onClick={e=>{e.stopPropagation();setSelected(p.id);}}>Edit Recipe</button>
                </div>
              </div>
            ))}
    ...
    <div className="fs-app">
      ...
      <aside className="fs-sidebar">
        <div className="fs-sidebar__brand">
          ...
          <div className="fs-sidebar__name">Farm<span>Sense</span> AI</div>
        </div>
